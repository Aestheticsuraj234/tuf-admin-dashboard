...
export const getDsaSheets = async()=>{
  try {
    ...
    const transformedDsaSheets = dsaSheets.map((sheet)=>{
      ...
      const chapterCount = sheet.dsaSteps.reduce((totalChapters , step)=>
      totalChapters + step._count.dsaChapters,0)
...
export const update_dsa_sheet = async (values: z.infer<typeof DsaContentSchema>, sheetId: string) => {
  const user = await currentUser();
  ...
  for (const stepData of dsaSteps) {
    ...
    if (existingStep) {
      await UpdateDsaStepByStepId(sheetId, existingStep.id, { dsaSteps: [stepData] });
    } else {
